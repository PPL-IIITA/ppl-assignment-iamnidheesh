	...
	for g in  G:
		for b in  B:
			...
			if(g.isElligible(b) and b.isElligible(g)):
				...
				g.changeStatus()
				b.changeStatus()
				logger(' commitment : ' +' day ' + str(i) + ' ' + g.name + " and " + b.name)
				...
				if(coupleHappiness < t):
					print("Break Up " + b.name+ " and " + g.name)
					logger("break up :(  : "  + b.name + " and " + g.name)
					C.pop(0)
					g.changeStatus()
					b.changeStatus()
				break
